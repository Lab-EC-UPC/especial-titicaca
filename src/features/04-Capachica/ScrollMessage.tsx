...
ScrollMessage.Wide = ({ start, end, children }: ScrollMessageProps) => {
    return (
        <div
            ...
            data-end={end}
        >
            <div className="relative flex w-full max-w-[min(92vw,1200px)] items-center justify-center text-white">
                <div
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-[-3.5rem] -z-10 rounded-[999px] bg-black/28 blur-[52px] opacity-90"
                />
                <div className="relative z-10 max-w-full drop-shadow-[0_16px_26px_rgba(0,0,0,0.92)] [&_img]:block [&_img]:max-w-full [&_img]:rounded-[1.25rem]">
                    {children}
                </div>
...
ScrollMessage.Image = ({ src, alt }: ScrollImageProps) => (
    <img src={src} alt={alt} className="mx-auto max-h-[50vh] w-auto" />
);
